...
<html lang="en">
	<head>
		...
		<script type="module">{[
			...
			`	alert("Your browser is not supported. If you are using Firefox, enable 'layout.css.at-scope.enabled' in about:config.");`,
			`}`,
			``,
		].join("\n")}</script>
	</head>
			...
			<ul>
				{...dailyThingsDone.map(({ date, thingsDone$ }) => {
					...
					const submit = () => {
						...
						thingsDone$ = [...thingsDone$, newItemInput.textContent];
							...
							<ul>
								{...thingsDone$.map((thingDone, index) => <li className="list-item">
									<Input initialValue={thingDone} on:blur={function () {
										// Currently extremely inefficient until winzig's reactivity gets better :/
										if (this.textContent !== thingDone) {
											thingsDone$ = thingsDone$.with(index, thingDone = this.textContent);
										}
									}}>{thingDone}</Input>
									<button on:click={() => thingsDone$ = thingsDone$.toSpliced(index, 1)}>x</button>
								</li>)}
								{css`
									& {
										display: contents;
